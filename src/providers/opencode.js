...
async function invokeCLI(prompt, args, env) {
  ...
  return new Promise((resolve) => {
    ...
    const cmd = (isWin && npmBin) ? path.join(npmBin, "opencode.cmd") : "opencode";
    ...
    const child = spawn(cmd, ["run", prompt, ...(args ?? [])], {
      cwd: process.cwd(),
      env: resolvedEnv,
...
function extractSummary(result) {
  ...
  return `OpenCode completed with exit code ${result.exitCode}`;
}
