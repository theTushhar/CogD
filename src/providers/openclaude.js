...
export function register(registry) {
  ...
  registry.register("openclaude", adapter);
}
...
async function invokeCLI(prompt, args, env) {
  ...
  return new Promise((resolve) => {
    ...
    const cmd = (isWin && npmBin) ? path.join(npmBin, "openclaude.cmd") : "openclaude";
...
function extractSummary(result) {
  ...
  return `OpenClaude completed with exit code ${result.exitCode}`;
}
